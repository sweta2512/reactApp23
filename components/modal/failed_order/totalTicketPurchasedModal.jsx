...
const TotalTicketPurchasedModal = ({ handleClose, show}) => {
  const dispatch = useDispatch();
  const [tableData, setTableData] = useState([]);
  const {id,name} = useSelector((state)=>state.showHideModal?.failed_order_total_ticket_purchase_modal_data);
  const { ticket_purchase_table_data } = useSelector((state) => state?.modal);

  const { showTotalPurchasedCreditModal } = useSelector((state) => state.failedOrder);
useEffect(() => {
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  dispatch(
                    ...
                    dispatch(
                      showCreditModal({
                        modal: "total_purchase_tickets_credit_card",
                      })
                    );
                  });
                }}
...
const calculateRowSpan = (data, index, key) => {
  ...
  return (
    <>
        ...
        <Modal.Header closeButton>
          <Modal.Title>
            <span className="modalTitle">
              Purchased Ticket Summary <span className="warning">{name}</span>
            </span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      <CreditCardModal
        show={showTotalPurchasedCreditModal}
        handleClose={() => {
          dispatch(hideCreditModal({ modal: "total_purchase_tickets_credit_card" }));
        }}
      />
    </>
  );
...
export default TotalTicketPurchasedModal