...
export function PurchaseCreditModal({ handleClose, show }) {
  const dispatch = useDispatch();
   
  let toast = useRef(null);
  const { id, eventName } = useSelector(
    (state) => state.showHideModal?.performancePurchaseData
  );

  const {cardID, cardName} = useSelector((state) => state?.performance?.purchaseCreditData);
  ...
  useEffect(() => {
    if (show === true) {
      dispatch(readCreditCardAction({ mpID: 1, ccID: cardID }));
    }
  }, [dispatch, show]);
  ...
  useEffect(() => {
    ...
    setZipcode(zip || "");
    setPhoneNo(phone || "");
    let country = loction?.country?.selected?.[0]?.split(", ");
    let state = loction?.state?.selected?.[0]?.split(", ");
    let city = loction?.city?.selected?.[0]?.split(", ");
  ...
  useMemo(() => {
    let city = loction?.city?.options;
    let state = loction?.state?.options;
    let country = loction?.country?.options;
  ...
  }, [loction, activeAccount, allLinkedAccount]);
   ...
   const handleUpdate = () => {
    ...
    if (cardExpiry?.length === 5) {
     ...
     console.log(expiry,'expiry',year,'year',cardExpiry);
    //return;
    dispatch(
      updateCreditCardAction({
        //ucID: id,
        ucID: cardActiveAccount?.code||"",
