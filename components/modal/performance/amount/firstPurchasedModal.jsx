...
export function AmountFirstPurchaseModal({ handleClose, show }) {
  const dispatch = useDispatch();
  const { id, eventName } = useSelector(
    (state) => state.showHideModal?.performance_amount_firstPurchase_data
  );

  let data1 = useSelector((state) => state.showHideModal?.performance_amount_firstPurchase_data);
  
 
  const { purchaseDetail } = useSelector((state) => state?.performance);
  const [data, setData] = useState([]);
  console.log(purchaseDetail, "performance_amount_firstPurchase_data" ,data1
  );
  useEffect(() => {
    if (show === true) {
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <span>
              First purchase for
              <br /> Plugin User:<span className="warning">{purchaseDetail?.title?.user}</span>
              <br />
              <span>
