...
import { AmountCreditModal } from "./creditModal";

export function AmountPurchaseModal({ handleClose, show }) {
  const dispatch = useDispatch();
  const { id, eventName } = useSelector(
    (state) => state.showHideModal?.performance_amount_purchase_data
  );
  const { purchaseDetail ,showAmountCreditModal } = useSelector((state) => state?.performance);
          ...
          <>
            ...
            record?.cardName !== 0 ? (
              <Button
                ...
                id="forgetButton"
                onClick={() => {
                  dispatch(showCreditModal({ modal: "amount", data: record }));
                }}
              >
                {record?.cardName}
              </Button>
  ...
  return (
    <>
      ...
      {/*  */}
      <AmountCreditModal
        show={showAmountCreditModal}
        handleClose={() => dispatch(hideCreditModal({ modal: "amount" }))}
      />
    </>
  );
}
