...
import { TicketCreditModal } from "./creditModal";

export function TicketsPurchaseModal({ handleClose, show }) {
  const dispatch = useDispatch();
  const { id, eventName } = useSelector(
    (state) => state.showHideModal?.performance_ticket_purchase_data
  );
  const { purchaseDetail, showTicketCreditModal } = useSelector((state) => state?.performance);
  const [data, setData] = useState([]);
  console.log(
    purchaseDetail,
    "performance_amount_firstPurchase_data",
    eventName
  );
  useEffect(() => {
    if (show === true) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {console.log(record,'record record')
        return (
          <>
            {record !== undefined &&
            record !== null &&
            record?.cardName !== 0 ? (
              <Button
                ...
                id="forgetButton"
                onClick={() => {
                  dispatch(showCreditModal({ modal: "ticket", data: record }));
                }}
              >
                {record?.cardName}
              </Button>
  ...
  return (
    <>
      ...
      {/*  */}
      <TicketCreditModal
        show={showTicketCreditModal}
        handleClose={() => {
          dispatch(hideCreditModal({ modal: "ticket" }));
        }}
      />
    </>
  );
