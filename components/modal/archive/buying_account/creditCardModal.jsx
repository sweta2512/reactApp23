...
const ArchiveBuyingAccountCreditCardModal = ({ handleClose, show }) => {
  const dispatch = useDispatch();
  let toast = useRef(null);
  const [mpID, setMpID] = useState(null);
  const { id, mp , card } = useSelector(
    (state) => state?.archiveBuyingAccount?.credit_card_data
  );
  ...
  useEffect(() => {
    if (show === true) {
      dispatch(readCreditCardAction({ mpID: mpID, ucID: id }));
    }
  ...
  const [nameOnCreditCard, setNameOnCreditCard] = useState(nameOnCard || "");
  const [cardNumber, setCardNumber] = useState(nameNumber || "");
  ...
  useEffect(() => {
    ...
    setCardCvv(cvv);
    // let expirydate;
    // let expiryLength= expiry?.length;

    // if(expiryLength>=7){
    //   let month = moment(expiry, ["YYYY-MM-DD"]).format("MM");
    //   let year = moment(expiry, ["YYYY-MM-DD"]).format("YY");
  ...
  const handleUpdate = () => {
    if(!cardExpiry){
      ...
      if (!nameOnCard) {
        showError("Name on card is requird.", toast);
        return false;
      }
      if (!cardNumber) {
        showError("Card Number is requird.", toast);
        return false;
      }

      if (!cardCvv) {
        showError("CVV is requird.", toast);
        return false;
      }
  
      if (!expiry) {
        showError("Card Expiry is requird.", toast);
        return false;
      }
  
      if (!cardActiveAccount?.code) {
        showError("Active account is requird.", toast);
        return false;
      }
  
      if (!cardAccountPriority?.name) {
        showError("Priority account is requird.", toast);
        return false;
      }
  
      if (!addressOne) {
        showError("Address 1  is requird.", toast);
        return false;
      }
      ...
      if (!selectedCity?.code) {
        showError("City is requird.", toast);
        return false;
      }
      
      if (!selectedState?.code) {
        showError("State is requird.", toast);
        return false;
      }
       
      if (!selectedCountry?.code) {
        showError("Country is requird.", toast);
        return false;
      }
  
      if (!zipcode) {
        showError("Zip code is requird.", toast);
        return false;
      }
  
      if (!phoneNo) {
        showError("Phone No. is requird.", toast);
        return false;
      }
...
export default ArchiveBuyingAccountCreditCardModal;
