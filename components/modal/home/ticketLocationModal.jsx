...
export const TicketLocationModal = React.memo(({ handleClose, show }) => {
  const dispatch = useDispatch();
  const [eventName ,setEventName] = useState('');
  const [history, setHistory] = useState([]);
  const { id, name } = useSelector((state) => state.modal.eventDetali);
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            Ticket Location <span style={{ color: " #F80" }}>{name}</span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
